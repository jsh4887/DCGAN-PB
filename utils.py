...
def blend(img_target, img_source, img_mask, offset=(0, 0)):
    ...
    for num_layer in range(img_target.shape[2]):
        ...
        img_target = img_target.reshape(region_source[2]-region_source[0], region_source[3]-region_source[1])
        img_source = img_source.reshape(region_source[2]-region_source[0], region_source[3]-region_source[1])

        img_target[img_target == 9999.] = 0.
        img_target = img_target + img_mask * img_source
        
        #img_target[5:(64-5), 5:(64-5)] = x[5:(64-5), 5:(64-5)] 
        img_target[region_source[0]:region_source[2], region_source[1]:region_source[3]] = x[region_source[0]:region_source[2], region_source[1]:region_source[3]]
        ###
    return img_target
...
# =============================================================================
    ## image size = 64
    #region_source = (0, 0, 64, 120)
    #region_target = (0, 0, 64, 120)
    #region_size = (region_source[2]-region_source[0], region_source[3]-region_source[1]) # 64, 64

    #img_mask = prepare_mask(img_mask)
    ####### extend west and east margins
    #img_target_ext = np.zeros([64, 120, 1]).astype(type(img_target[0, 0, 0]))
    #img_target_ext[:, 0:28] = img_target[:, 36:64]
    #img_target_ext[:, 28:92] = img_target
    #img_target_ext[:, 92:120] = img_target[:, 0:28]
   
    #img_mask_ext = np.zeros([64, 120]).astype(np.int8)
    #img_mask_ext[:, 0:28] = img_mask[:, 36:64]
    #img_mask_ext[:, 28:92] = img_mask
    #img_mask_ext[:, 92:120] = img_mask[:, 0:28]

    #img_source_ext = np.zeros([64, 120, 1]).astype(type(img_source[0, 0, 0]))
    #img_source_ext[:, 0:28] = img_source[:, 36:64]
    #img_source_ext[:, 28:92] = img_source
    #img_source_ext[:, 92:120] = img_source[:, 0:28]
    
    # image size = 32
    reg_s_1 = region_source[0]
    reg_s_2 = region_source[1]
    reg_s_3 = region_source[2]
    reg_s_4 = region_source[3]
    ext_reg_s = 90
    ext_reg_s_3 = reg_s_3 + ext_reg_s
    ext_reg_s_4 = reg_s_4 + ext_reg_s
    
    region_source = (reg_s_1, reg_s_2, ext_reg_s_3, ext_reg_s_4)
    region_target = (reg_s_1, reg_s_2, ext_reg_s_3, ext_reg_s_4)
    region_size = (region_source[2]-region_source[0], region_source[3]-region_source[1]) # 64, 64
    #import pdb; pdb.set_trace() 
    img_mask = prepare_mask(img_mask)
    ####### extend west and east margins
    img_target_ext = np.zeros([ext_reg_s_3, ext_reg_s_4, 1]).astype(type(img_target[0, 0, 0]))
    img_mask_ext = np.zeros([ext_reg_s_3, ext_reg_s_4]).astype(np.int8)
    img_source_ext = np.zeros([ext_reg_s_3, ext_reg_s_4, 1]).astype(type(img_source[0, 0, 0]))
    
    for i in range(0, int(ext_reg_s/2)):
        img_target_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), i] = img_target[:, 0]
        img_mask_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), i] = img_mask[:, 0]
        img_source_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), i] = img_source[:, 0]
        
        img_target_ext[i, int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)] = img_target[0, :]
        img_mask_ext[i, int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)] = img_mask[0, :]
        img_source_ext[i, int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)] = img_source[0, :]

    img_target_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)] = img_target
    img_mask_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)] = img_mask
    img_source_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)] = img_source

    for j in range(int(ext_reg_s_4)-int(ext_reg_s/2), int(ext_reg_s_4)):
        img_target_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), j] = img_target[:, int(reg_s_3)-1]
        img_mask_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), j] = img_mask[:, int(reg_s_3)-1]
        img_source_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), j] = img_source[:, int(reg_s_3)-1]
...
# =============================================================================
       ...
       # import pdb; pdb.set_trace()
    return img_target_ext[int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2), int(ext_reg_s/2):int(ext_reg_s_4)-int(ext_reg_s/2)].reshape(int(reg_s_3), -1)
